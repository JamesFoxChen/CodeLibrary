...
namespace Qiniu.IO
{
    ...
    public class IOClient
    {
        ...
        public PutRet PutFile(string upToken, string key, string localFile, PutExtra extra)
        {
            ...
            try
            {
                CallRet callRet = MultiPart.MultiPost(QiniuConfig.UP_HOST, formData, localFile, this.Proxy);
        ...
        public PutRet Put(string upToken, string key, Stream putStream, PutExtra extra)
        {
            ...
            try
            {
                CallRet callRet = MultiPart.MultiPost(QiniuConfig.UP_HOST, formData, putStream);
                ret = new PutRet(callRet);
                onPutFinished(ret);
                return ret;
            }
