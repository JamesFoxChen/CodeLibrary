...
namespace Qiniu.RS
{
    ...
    public class Pfop : QiniuAuthClient
    {
        ...
        public string Do(EntryPath entry, string[] fops, Uri notifyURL,string pipleline,int force=0)
        {
            ...
            CallRet ret = CallWithBinary(QiniuConfig.API_HOST + "/pfop/", "application/x-www-form-urlencoded", StreamEx.ToStream(body), body.Length);
        ...
        {
            CallRet ret = Call(string.Format("{0}/status/get/prefop?id={1}", QiniuConfig.API_HOST, persistentId));
            if (ret.OK)
            {
                return ret.Response;
            }
