...
namespace Qiniu.Auth.digest
{
	...
	public class Mac
	{
		...
		public Mac ()
		{
            ...
            this.secretKey = QiniuConfig.Encoding.GetBytes(QiniuConfig.SECRET_KEY);
		}
		...
		public string SignWithData (byte[] b)
		{
            ...
            return string.Format("{0}:{1}:{2}", this.accessKey, _sign(QiniuConfig.Encoding.GetBytes(data)), data);
		}
		...
		public string SignRequest (System.Net.HttpWebRequest request, byte[] body)
		{
			...
			using (HMACSHA1 hmac = new HMACSHA1(secretKey)) {
                ...
                byte[] pathAndQueryBytes = QiniuConfig.Encoding.GetBytes(pathAndQuery);
